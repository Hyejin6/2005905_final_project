...
from .models import Post, Comment # Post 사용하기 위해 정의
...
def post_list(request): # 목록에 대한 뷰를 request를 받는 함수형으로 정의
    ...
    posts = Post.objects.filter(published_date__lte=timezone.now()).order_by('-created_date') # 최근 글이 위로
...
# 댓글 기능
def add_comment_to_post(request, pk): # 함수형으로 정의 - 댓글 추가
    post = get_object_or_404(Post, pk=pk)
    if request.method == "POST": # request에서 가지고 있는 메서드가 POST이면
        form = CommentForm(request.POST) # 사용자가 form에 제목이나 글을 입력을 하고 저장을 하면 request 메서드가 POST로 전달이 됨
        if form.is_valid(): # form의 값들이 바른지 체크
            comment = form.save(commit=False) # 받아온 값들을 저장 - commit=False는 바로 저장하지 말라는 것
            comment.post = post
            comment.save() # 저장버튼 - 실제로 이 때 db에 저장이 된다
            return redirect('blog:post_detail', pk=post.pk) # redirect 메서드 호출해서 post_detail로 연결
    else: # request에서 가지고 있는 메서드가 POST가 아니면
        form = CommentForm() # form에 CommentForm()을 할당 - 즉 비어있는 댓글 form을 보여주라는 뜻
    return render(request, 'blog/add_comment_to_post.html', {'form': form}) # add_comment_to_post.html을 리턴하고 form 변수에 form을 할당
